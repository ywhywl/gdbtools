...
func TestParseArgs_ValidationErrors(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "drop with new-dbname",
			command: "drop",
			args:    []string{"--host", "localhost", "--old-dbname", "test_bak", "--new-dbname", "test2"},
			wantErr: "--new-dbname is not applicable in drop mode",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := parseArgs(tt.command, tt.args)
			if err == nil {
