...
最小身份单位是 `user@host`，统一映射到 `PrivilegeSnapshot`：

- 全局权限：`GlobalPrivileges`
- 库级权限：`DBPrivileges`
- 表级权限：`TablePrivileges`

规则检查通过快照数组完成，不依赖源端和目标端的比较模型。
...
- 同一个 `user@host` 同时拥有多个 schema 的库级权限
