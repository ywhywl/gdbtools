...
每行集群的 M、S、LS、OS、TS 展开后 IP 总数最多为 10，恰好 10 个允许创建。该上限按集群独立计算，不限制整个 CSV 的 IP 总数。

超过 10 个时在 CSV 校验阶段整批报错退出（退出码 3），不进行 SSH 检测或提交任何集群创建请求。`--dry-run` 同样执行此校验。错误包含行号、集群名称、实际数量和上限，例如：

```text
第 2 行集群 cluster_11 IP 数量为 11，最多支持 10 个 IP（每个 IP 占用一个 teamId）
```
...
每个 IP 独立生成一个 team。按角色顺序 `M -> S -> LS -> OS -> TS` 展开，角色内按输入顺序分配连续的 teamId：
...
- 不按角色或 IDC 重置；
- 不区分 `;` 或 `|`；
- 第 N 个展开的 IP 使用 `teamId=N`；
- Insight 只支持 `1` 到 `10`，同一集群 IP 数量超过 10 时在请求前直接报错。

当五个角色均存在且各有一个 IP 时，映射为 M=1、S=2、LS=3、OS=4、TS=5。空角色不预留编号，例如仅有 M、S、OS 且各有一个 IP 时，teamId 分别为 1、2、3。teamId 不再与角色固定绑定。
...
假设前面的 M、S、LS 各有一个 IP，则上述 OS 节点生成：
...
后续 IP 不会因为使用 `;` 而重新成为逻辑主。OS 首个 IP 的 teamId 取决于前面角色的 IP 数量，其逻辑主身份由 `dbRole=2` 表示。
...
以下为前面的 M、S、LS 各有一个 IP 时的 OS team 请求体片段（省略其他角色）：
...
| --- | ---: | --- |
| 物理机 | 任意 | `pm` |
| 虚拟机 | < 22 | 报错；仅在类型不一致且指定允许参数时使用 `vm_l` |
| 虚拟机 | >= 22 且 < 30 | `vm_l` |
| 虚拟机 | >= 30 且 < 46 | `vm_m` |
| 虚拟机 | >= 46 | `vm_h` |

虚拟机内存低于 22G 时默认按原业务逻辑因内存阈值报错退出。已有的 `--allow-low-memory-vm` 逻辑继续保留：指定后可降级使用 `vm_l`。此外，只有当一个集群包含多个节点、节点检测出的 `server_type` 不一致，并且指定 `--allow-server-type-mismatch` 时，才允许忽略该类型差异；如果不一致场景包含低内存虚拟机，则使用 `vm_l` 模板继续。
...
如果不一致场景中包含内存低于 22G 的虚拟机，则使用 `vm_l` 模板。若存在低于 22G 的虚拟机但节点类型一致，`--allow-server-type-mismatch` 本身不会放宽内存阈值；此时仍需依靠原有的 `--allow-low-memory-vm` 逻辑。
...
3. 读取 CSV
4. 按 ; 和 | 展开各角色 IP
5. 校验 M 单 IP、空元素、单集群最多 10 个 IP 和全局 IP 唯一性；任一行失败则整批退出
6. 自动检测全部主机并选择 server_type
7. 校验主机 server_type 一致性，或按参数告警继续
8. 为所有 IP 顺序分配 1 到 10 的 teamId
9. 构建 DN/CN 请求体
...
- 五个角色各单 IP 时分配 teamId 1～5，缺少角色时编号连续；
- `;`、`|` 及混合分隔符；
- M 多 IP报错；
- 空元素报错；
- 集群全局 IP 重复报错；
...
- OS 所有节点使用 OS DN 模板；
- 每 IP 一个 team；
- teamId 按 IP 展开顺序从 1 连续递增，最大为 10；
- 集群恰好 10 个 IP 时成功，超过 10 时在 SSH 检测和请求前整批报错；
- 多集群独立校验数量，teamId 各自从 1 开始；
- OS 与 TS 同时配置；
- 虚拟机内存低于 22G 默认报错；保留 `--allow-low-memory-vm` 原有降级逻辑；类型不一致且指定 `--allow-server-type-mismatch` 时也可使用 `vm_l`；
...
| teamId | 角色固定为 1～5 | 按 IP 展开顺序连续分配 1～10 |
| OS 逻辑主 | 单 IP时为 teamId=4 | OS 第一个 IP 的 dbRole=2，teamId 按集群 IP 顺序分配 |
